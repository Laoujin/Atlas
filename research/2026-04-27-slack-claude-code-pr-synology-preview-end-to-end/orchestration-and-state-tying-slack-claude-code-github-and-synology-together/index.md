...
|---|---|---|---|
| **Redis** | Anywhere (Synology container, [Upstash](https://upstash.com), [Fly](https://fly.io)) | Lots of ephemeral keys, distributed locks per Slack thread [[21]](https://redis.io/tutorials/chat-sdk-slackbot-distributed-locking/) | Leases prevent two workers replying to the same thread; TTL deletes stale state |
...
| **[Inngest](https://www.inngest.com)** [[9]](https://github.com/inngest/inngest) | Cloud (proprietary) or self-host single-node SQLite [[24]](https://railway.com/deploy/inngest-single-node-sqlite) | Calls into your serverless endpoints | Step-level retries; event-driven first | Event-driven backends; serverless | ⭐ 5.3k |
| **[Trigger.dev](https://trigger.dev) v3** [[10]](https://github.com/triggerdotdev/trigger.dev) | Postgres + Redis | Cloud or self-host (Apache 2.0) | Checkpoint/resume on dedicated compute, no serverless timeout [[12]](https://medium.com/@matthieumordrel/the-ultimate-guide-to-typescript-orchestration-temporal-vs-trigger-dev-vs-inngest-and-beyond-29e1147c8f2d) | TS-first teams that want best DX | ⭐ 15k |
| **[Hatchet](https://hatchet.run)** [[11]](https://github.com/hatchet-dev/hatchet) | Postgres | Self-host with one DB | DAGs, rate limits, concurrency keys, >100 jobs/s | High-throughput task queues | ⭐ 7.0k |
| **[Windmill](https://www.windmill.dev)** [[25]](https://github.com/windmill-labs/windmill) | Postgres | Self-host single binary or Docker | One ACID statement per state transition | Internal-tools / scripts platform | ⭐ 16k |
| **[Dagu](https://dagu.sh)** [[23]](https://dagu.sh) | None required (filesystem) | Single binary, no DB | DAGs over scripts, containers, SQL, HTTP | Cron + approvals on a homelab | ⭐ — |
...
- **DEEs and event logs are complements, not substitutes.** If [Kafka](https://kafka.apache.org) is already in the stack, the engine handles the workflow lifecycle on top of the event log [[28]](https://www.kai-waehner.de/blog/2025/06/05/the-rise-of-the-durable-execution-engine-temporal-restate-in-an-event-driven-architecture-apache-kafka/).
