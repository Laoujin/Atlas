...
**Local-first AI is the second cross-cutting thread.** Ollama + `nomic-embed-text` + Llama 3.1 8B (or Qwen 2.5 32B for quality) is the free backbone — it runs the AI Tagger Universe retro-fill, embeds for Smart Connections discovery, and chats through Copilot's `localhost:11434` adapter using the documented `OLLAMA_ORIGINS=app://obsidian.md*` workaround for CORS [[9]](https://github.com/logancyang/obsidian-copilot/blob/master/local_copilot.md). One subtle warning recurs: **don't double-index**. Running Smart Connections + Copilot Vault QA + Smart Composer RAG simultaneously means three separate embedding indexes over the same notes [[10]](https://forum.obsidian.md/t/alternatives-to-smart-connections/108886). Pick one chat plugin; let Smart Connections own the discovery sidebar.
